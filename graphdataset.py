...
def create_amazonwiki_graph(file_one=None, file_two=None):
    ...
    g = Data(train_data=train_data, test_data=test_data, val_data=val_data, metadata=metadata, adj_a=adj_a, adj_b=adj_b)
    ...
    g.x = torch.cat([torch.FloatTensor(graph_a.features), torch.FloatTensor(graph_b.features)], 0)
    ...
    g.metadata = {}
    g.metadata["num_type_a"] = num_type_a
    g.metadata["num_type_b"] = num_type_b
    g.metadata["type_a_dict"] = type_a_dict
    g.metadata["type_b_dict"] = type_b_dict
    g.metadata["num_nodes_a"] = len(graph_a.id2idx)
    g.adj_a = adj_a
    g.adj_b = adj_b
    ...
    torch.save(g, "amazon_wiki_pyg_1")
