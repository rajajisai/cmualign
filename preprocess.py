...
class Graph(object):
	...
	def   buildGraph(self, table):
		...
		with open(table,encoding="utf8") as IN:
			...
			self.edge_src=torch.LongTensor(self.edge_src)
			self.edge_src=torch.reshape(self.edge_src,(self.edge_src.shape[0],1))
			self.edge_dst=torch.LongTensor(self.edge_dst)
			self.edge_dst=torch.reshape(self.edge_dst,(self.edge_dst.shape[0],1))
			#for tmp in triples
