        ...
        for i in range(train_data.shape[0]):
            ...
            for m in random.sample(adj_b[train_data[i, 1]], min(num_neighbors, len(adj_b[train_data[i, 1]]))):
                ...
                for sub_edge in type_b_dict[(m, train_data[i, 1])]:
                    edge_indices[sub_edge + 1].append(g.edge_id(m+len(graph_a.id2idx),
                                                      train_data[i, 1]+len(graph_a.id2idx)))
...
def genSubGraph(graph_a, graph_b, num_hops=1):
    ...
    g.ndata['features'] = torch.cat([torch.FloatTensor(graph_a.features),
...
def main(args):
    ...
    train_data, val_data, test_data = generateTrainWithType(
        'data/itunes_amazon_exp_data/exp_data/', graph_a, graph_b, positive_only=args.model_opt == 0)
    # print(train_data)
    if args.gpu < 0:
        cuda = False
    else:
    ...
    for epoch in range(args.n_epochs):
        ...
        for batch in train_loader:
            ...
            # embed()
            if False:
                output_a = emb[batch[:, 0]].view(-1, args.num_negatives+1, 2 * args.n_hidden)
                output_b = emb[batch[:, 1] + offset].view(-1, args.num_test_negatives+1, 2 * args.n_hidden)
                # g.remove_edges(eid)
